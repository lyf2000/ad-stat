...
class FirstStepPaymentInvoiceNotifierMessageBuilderService(CoreClickRuNotifierMessageBuilderService):
    ...
    def get_head_msg(self) -> TgMessageItem:
        msg = TgMessageItem(
            msg=(
                f"Доброе утро, {self.tag_user(self.company.payment_setting.tag_tg_user())}!"
                " Высылаем счет на пополнение рекламного кабинета."
                " Необходимо оплатить его в течение 5 рабочих дней"
...
class FirstStepPaymentInvoiceNotifierService(BaseTgNotifierService):
    STAT_TYPE = NotieLogTypes.PAYMENT_INVOICE_FIRST_STEP
    ...
    def send_notie(self) -> None:
        with NotieDeliverLogFlow(company=self.company, type=self.STAT_TYPE) as deliver:
            msg = FirstStepPaymentInvoiceNotifierMessageBuilderService(company=self.company).compose_msg(separated=True)

...
class BaseDataCacheCollector(CoreDataCacheCollector):
    ...
    def get_data(self):
        if not self.has_key(self.get_key()):
            self.set_data()
...
# multi keys
class BaseDataKeysCacheCollectorV2(CoreDataCacheCollector):
    KEY_TEMPLATE = ""
    AUTO_COLLECT = True
    ...
    def get_key(self):
        return self.KEY_TEMPLATE.format(**self.get_keys_init())

    def get_data(self):
        if not self.has_key(self.get_key()) and self.AUTO_COLLECT:
            self.set_data()

...
from ad_stat.integrations.api.base import request as custom_request
    ...
    def methods(self) -> tuple:
        ...
        for attr_name in dir(api):
            ...
            if callable(attr_value) and (
                custom_request in getattr(attr_value, "_marked", []) or request in getattr(attr_value, "_marked", [])
            ):
...
def _handle_response(content: dict, response_parser, detail_response_parser, response_model):
    if response_parser:
